...
// WuXiaolong@Multimedia 2023/12/26 Add for TPCharger
#define FTS_PROC_CHARGE_DETECT_FILE	"charge_detect"
struct proc_dir_entry *fts_proc_charge_detect_file;
...
//xiazhiping@
static ssize_t fts_black_screen_test_write(struct file *file, const char *buff,
									size_t len, loff_t *pos)
{
	...
	if (buf[0] == '0') {
		touch_black_test = 0;
        FTS_TEST_INFO("fts_black_screen_test_write 0");
	}
	else if (buf[0] == '1') {
        FTS_TEST_INFO("fts_black_screen_test_write 1");
		touch_black_test = 1;
	}
	return len;
}
...
{
    FTS_TEST_INFO("tp_gesture_open open!\n");
...
// WuXiaolong@Multimedia 2023/12/26 Add for TPCharger,start
...
static int tp_charger_mode_open(struct inode *inode, struct file *file)
{
    FTS_TEST_INFO("tp_charger_mode open!\n");
    return single_open(file, tp_charger_mode_read_func, PDE_DATA(inode));
}

static ssize_t tp_charger_mode_write(struct file *file, const char *buff,
                                size_t len, loff_t *pos)
{
    int ret = 0;
    char buf[80] = {0};
...
static const struct file_operations tp_charger_mode_proc_fops = {
        .open = tp_charger_mode_open,
        .read = seq_read,
        .write = tp_charger_mode_write,
        .release = single_release,
};
// WuXiaolong@Multimedia 2023/12/26 Add for TPCharger,end
...
static int tp_headset_mode_open(struct inode *inode, struct file *file)
{
    FTS_TEST_INFO("tp_headset_mode open!\n");
    return single_open(file, tp_headset_mode_read_func, PDE_DATA(inode));
}

static ssize_t tp_headset_mode_write(struct file *file, const char *buff,
                                     size_t len, loff_t *pos)
{
    int ret = 0;
    char buf[80] = {0};
...
static const struct file_operations tp_headset_mode_proc_fops = {
        .open = tp_headset_mode_open,
        .read = seq_read,
        .write = tp_headset_mode_write,
        .release = single_release,
};
