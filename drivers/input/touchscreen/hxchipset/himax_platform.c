...
static char firmware_version_hkc_hx05k[25] = {'H','U','L','K','_','K','_','L','C','E','_','H','K','C','_','H','X','0','5'};
...
void ovt_get_tp_fw_ver(void)
{
	...
	if (g_firmware_id == 0) {
		...
		firmware_version_hkc_hx05k[19] = str[0];
		firmware_version_hkc_hx05k[20] = str[1];
		get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version_hkc_hx05k);
		printk("firmware_version:%s\n",firmware_version_hkc_hx05k);
	}
}
...
extern int  get_ishulk_10w(void);
static int __init himax_common_init(void)
{
	int retval = 0;
	int   ishulk_10w = 0;
	retval = get_bootargs(lcd_name, "lcd_name");
	ishulk_10w = get_ishulk_10w();
